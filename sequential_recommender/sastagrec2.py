...
class SASTagRec2(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        #     nn.Linear(self.hidden_size*2, self.hidden_size*2), nn.ReLU(True), nn.Linear(self.hidden_size*2, self.hidden_size)
        # )
    ...
    def tag_infer_train(self, target_tags):
        ...
        if self.mode == 'user_tag':
            ...
            topk_ut_freq, topk_ut_indices = torch.topk(ut2it2ut, self.branch_factor, dim=1)
            # topk_ut_freq /= topk_ut_freq.sum(dim=-1, keepdim=True)
            return topk_ut_freq, topk_ut_indices
        else:
            it2ut = self.it2ut[target_tags] #[b, max_ut_len, n_it]
            it2ut2it = torch.matmul(it2ut, self.ut2it).sum(dim=1)  #[b, n_ut]
            topk_it_freq, topk_it_indices = torch.topk(it2ut2it, self.branch_factor, dim=1)
            # topk_ut_freq /= topk_ut_freq.sum(dim=-1, keepdim=True)
            return topk_it_freq, topk_it_indices
    ...
    def calculate_loss(self, interaction):
        ...
        topk_ut_freq_infer, topk_ut_indices_infer = self.tag_infer_train(target_tags)
        topk_ut_freq_infer_neg, topk_ut_indices_infer_neg = self.tag_infer_train(neg_target_tags)
        # all_target_tags = target_tags
        ...
        if self.mode == 'item_tag':
            tag_embds = self.item_tag_embeddings(all_target_tags)
            neg_tag_embds = self.item_tag_embeddings(all_target_tags_neg)
        elif self.mode == 'user_tag':
            tag_embds = self.user_tag_embeddings(all_target_tags) #[b, n_tags, h]
            neg_tag_embds = self.user_tag_embeddings(all_target_tags_neg)
        ...
        tag_logits = torch.mul(output.unsqueeze(1), tag_embds).sum(dim=-1) #[b, topk_tags]
        ...
        # combined_binary_item_tags = torch.cat((item_tag_logits, neg_item_tag_logits), dim=1)
        ...
        neg_item_tag_logits = torch.mul(self.item_embedding(pos_items).unsqueeze(1), neg_tag_embds).sum(dim=-1)  
    ...
    def full_sort_predict(self, interaction):
        ...
        if self.full_tag_infer:
            ...
            infer_user_tags = torch.matmul(infer_item_tags, self.it2ut)
            infer_user_tags /= (infer_user_tags.sum(dim=1, keepdim=True) + 1e-8)

            weighted_emb = (all_tag_emb.unsqueeze(0) * infer_user_tags.unsqueeze(-1)).sum(dim=1) #[b, hidden]
            # # selected_tag_emb = all_tag_emb.unsqueeze(0).expand(infer_user_tags.shape[0], -1, -1) # [b, n_ut, hidden]
            # # infer_user_tags_expanded = infer_user_tags.unsqueeze(-1)  # [b, n_ut, 1]
            # # weighted_emb = (selected_tag_emb * infer_user_tags_expanded).sum(dim=1) # [b, hidden]
            ...
            # infer_average_score = (infer_average_score * infer_user_tags.unsqueeze(-1)).sum(dim=1) # [b, n_items]
        else:
            ...
            # weighted_scores = (tag_item_scores * infer_tags_freq.unsqueeze(-1)).sum(dim=1)  # [b, n_items]
