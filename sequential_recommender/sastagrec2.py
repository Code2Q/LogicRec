...
class SASTagRec2(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        # self.tag_hidden_size = config["tag_hidden_size"]
        ...
        # self.ut2it2ut = torch.matmul(self.ut2it, self.it2ut)  # [max_ut, max_ut]
        # self.it2ut2it = torch.matmul(self.it2ut, self.ut2it)  # [max_it, max_it]
...
#             self.it2ut = torch.load('it2ut_sp.pt').to(self.device).to_dense()
        ...
        self.tag_position_embedding = nn.Embedding(self.max_seq_length, self.hidden_size) 
        ...
        self.feature_att_layer = VanillaAttention(self.hidden_size, 2 * self.hidden_size)
        self.feature_trm_encoder = TransformerEncoder(
            n_layers=self.n_layers,
            n_heads=self.n_heads,
            hidden_size=self.hidden_size, #  // 2 = 64
            inner_size=self.inner_size,
        ...
        self.TagLayerNorm = nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)
        self.dropout = nn.Dropout(self.hidden_dropout_prob)
        self.feat_dropout = nn.Dropout(self.feat_hidden_dropout_prob)
        self.id_proj = nn.Linear(self.hidden_size, self.hidden_size)
        self.tag_proj = nn.Linear(self.hidden_size, self.hidden_size)
        self.prediction_layer = nn.Linear(self.hidden_size + self.hidden_size, self.hidden_size)
    ...
    def forward(self, item_seq, item_seq_len, item_tag_id_lists):
        ...
        feature_emb = feature_emb + self.tag_position_embedding(position_ids)
        feature_emb = self.TagLayerNorm(feature_emb)
        ...
        # fused_output = self.id_proj(item_output) + self.tag_proj(feature_output)
        output_concat = torch.cat((item_output, feature_output), -1)  # [B 2*H]
        fused_output = self.prediction_layer(output_concat)
        
        return fused_output  # [B H]

    def tag_infer_train(self, target_tags): # input : [b, max_tag_len]
        if self.mode == 'user_tag':
            selected_ut2it = self.ut2it[target_tags]  # [B, max_tag_len, max_it]
            ut2it2ut = torch.matmul(selected_ut2it, self.it2ut)  # [B, max_tag_len, max_ut]
            ut2it2ut = ut2it2ut.sum(dim=1)
            ...
            selected_it2ut = self.it2ut[target_tags]  # [B, max_tag_len, max_ut]
            it2ut2it = torch.matmul(selected_it2ut, self.ut2it)  # [B, max_tag_len, max_it]
            it2ut2it = it2ut2it.sum(dim=1)
...
#         all_target_tags_neg = neg_target_tags
        ...
        reg_loss = self.reg_loss(self.item_embedding(pos_items), all_tag_embds, all_neg_tag_embds)

        loss = (1-self.lamda) * id_loss + self.lamda * (tag_loss + item_tag_loss) + self.reg_weight * reg_loss
    ...
    def infer_tags(self, all_tag_scores):
        if self.mode == 'user_tag':
            ...
            topk_ut_values = topk_ut_values / (topk_ut_values.sum(dim=1, keepdim=True) + 1e-8)

            selected_ut2it = self.ut2it[topk_ut_indices] #[b, topk_user_tags, n_items]
            aggregated_item_tags = torch.einsum('bti,bt->bi', selected_ut2it, topk_ut_values)
            ...
            aggregated_user_tags = torch.einsum('bti,bt->bi', selected_it2ut, topk_infer_item_tags)
            ...
            topk_it_values, topk_it_indices = torch.topk(all_tag_scores, 2*self.branch_factor, dim=1) #[b, topk_user_tags]
    ...
    def full_sort_predict(self, interaction):
        ...
        return id_scores + 0.1 * average_tag_scores  + 0.01 * infer_average_score
