...
class GRU4TagRec(SequentialRecommender):

    def __init__(self, config, dataset):
        super(GRU4TagRec, self).__init__(config, dataset)
    ...
    def forward(self, item_seq, item_seq_len, user_tag_id_lists):
        ...
        # non_zero_elements = last_column[last_column != 0]
        # item_tag_id_lists = self.pad_to_length(non_zero_elements, 200).unsqueeze(0) # 构建新的张量，形状为 (1, 200)
        it_seq_embedding = self.user_tag_embeddings(user_tag_id_lists) # ([1, 50, 15, 64]) 
        # print(f"item tag seq embd0 {it_seq_embedding.shape}")

        if self.pooling_mode == "max":
            it_seq_embedding = torch.max(it_seq_embedding, dim=-2)[0]
            # print(f"pooling {self.pooling_mode}")
    ...
    def calculate_loss(self, interaction):
        ...
        # print(f"item seq {item_seq.shape}") #torch.Size([512, 100])
        user_tag_id_lists = self.item_feat['user_tag_id_list'][item_seq] #(50, 15)
        seq_output = self.forward(item_seq, item_seq_len, user_tag_id_lists) #[b, embed_size]
        # print(f"seq outoput {seq_output.shape}") #torch.Size([512, 64])
        ...
        if self.loss_type == "BPR":
            ...
            # pos_it = self.item_feat['item_tag_id_list'][pos_items].nonzero(as_tuple=True)
            # pos_it = self.pad_to_length(pos_it, 200, 0)
            # neg_it = self.item_feat['item_tag_id_list'][neg_items].nonzero(as_tuple=True)
            # neg_it = self.pad_to_length(neg_it, 200, 0)
            # print(pos_items, pos_items.shape, neg_items, neg_items.shape) # torch.Size([512])
            ...
            pos_score = torch.sum(seq_output * pos_it_emb, dim=-1)  # [B]
            neg_score = torch.sum(seq_output * neg_it_emb, dim=-1)  # [B]
    ...
    def predict(self, interaction):
        ...
        user_tag_id_lists = self.item_feat['user_tag_id_list'][item_seq] #(50, 15)
        print(f"user tag list {user_tag_id_lists.shape}")
        seq_output = self.forward(item_seq, item_seq_len, user_tag_id_lists)
        test_item_emb = self.item_embedding(test_item)

        # item_tags = self.item_feat['item_tag_id_list'][test_item]
        # item_tag_embs = self.item_tag_embeddings(item_tags)
        user_tags = self.item_feat['user_tag_id_list'][test_item]
        user_tag_embs = self.user_tag_embeddings(user_tags)
        if self.pooling_mode == "max":
            item_tag_emb = user_tag_embs.max(dim=1)[0]
        elif self.pooling_mode == "sum":
            item_tag_emb = user_tag_embs.sum(dim=1)
        elif self.pooling_mode == "mean":
            item_tag_emb = user_tag_embs.mean(dim=1)
    ...
    def full_sort_predict(self, interaction):
        ...
        # item_tags = self.item_feat['item_tag_id_list'] #[n_items, 19]
        # item_tag_embs = self.item_tag_embeddings(item_tags) #[n_items, 19, 64]
        user_tags = self.item_feat['user_tag_id_list'] #[n_items, 19]
        user_tag_embs = self.user_tag_embeddings(user_tags) #[n_items, 19, 6
        ...
        scores = torch.matmul(
            seq_output, test_items_embeds.transpose(0, 1)
        )  # [B, n_items]
        return scores
